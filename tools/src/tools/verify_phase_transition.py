...
def run_finite_size_scaling(
    ...
    results.append(ScalingResult(
        "γ/ν from χ_max", slope, GAMMA_NU_2D, r2,
    ...
    results.append(ScalingResult(
        "β/ν from m(T_c)", -slope_m, BETA_NU_2D, r2_m,
...
def run_bld_link_divergence(
    ...
    # Filter: ξ in reliable range (> 2 to avoid noise, < L/4 to avoid saturation)
    ...
    if len(usable) >= 3:
        ...
        slope, _, r2 = _linfit(x, y)
        results.append(ScalingResult(
            "ν from L_bld(2D)", slope, NU_2D, r2,
...
# ---------------------------------------------------------------------------
# Test 3: Binder cumulant crossing (2D)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Test 5: Nearest-neighbor link profile (2D)
# ---------------------------------------------------------------------------
...
def run_nn_link_profile(
    ...
    # L_nn should decrease with T (ρ_nn monotonically decreases T=0→∞)
...
# ---------------------------------------------------------------------------
# Test 6: 3D Ising — discriminating test
# ---------------------------------------------------------------------------
...
def run_3d_ising(
    ...
    # Fit ν from L_bld vs ln(1/|t|)
    # L_bld = ln(ξ) = -ν ln|t| + const  →  ln(ξ) vs ln(1/|t|) slope = ν
    slope_nu, _, r2_nu = _linfit(ln_inv_t, ln_xi)
    results.append(ScalingResult(
        "ν from 3D", slope_nu, NU_3D, r2_nu,
        abs(slope_nu - NU_3D) < 0.15 and r2_nu > 0.90,
    ))

    # Discriminating test: L_from_t - L_A should be constant
    # L_from_t = -ν ln|t| = ν ln(1/|t|)
    L_from_t = NU_3D * ln_inv_t
    L_A = ln_xi  # scenario A: L = ln(ξ)
    L_B = NU_3D * ln_xi  # scenario B: L = ν ln(ξ)
...
def main() -> int:
    ...
    print("=" * 80)
    print("PHASE TRANSITION VERIFICATION — BLD LINK DIVERGENCE")
    print("=" * 80)

    groups = [
        ("FINITE-SIZE SCALING (2D)", run_finite_size_scaling),
        ("BLD LINK DIVERGENCE (2D)", run_bld_link_divergence),
        ("BINDER CROSSING (2D)", run_binder_crossing),
        ("D×L INTERPLAY (2D)", run_dl_interplay),
        ("NN LINK PROFILE (2D)", run_nn_link_profile),
        ("3D ISING — DISCRIMINATION", run_3d_ising),
    ]
    ...
    for name, fn in groups:
        ...
        for r in results:
            total += 1
            if isinstance(r, ScalingResult):
                status = "PASS" if r.passes else "FAIL"
                print(
                    f"  {r.name:30s}: fitted={r.fitted:.4f}, "
                    f"exact={r.exact:.4f}, R²={r.r_squared:.3f} [{status}]"
                )
            elif isinstance(r, LinkResult):
                status = "PASS" if r.passes else "FAIL"
                print(
                    f"  T={r.T:.4f}: ξ={r.xi:.3f}, "
                    f"ρ={r.rho:.4f}, L={r.L_bld:.4f} [{status}]"
                )
            if r.passes:
                passed += 1
            else:
