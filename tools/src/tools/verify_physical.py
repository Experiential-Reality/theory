...
def _process_job(
    ...
    for g_tau in g_tau_values:
        ...
        pointers = quantum.PointerSet(
            (A0_prime, A1_prime),
            quantum.PointerKind.NON_ORTHOGONAL,
            overlap_epsilon=eps,
        )
        outcome = quantum.run_selection_mc(
    ...
    for alphas_sq in alpha_configs:
        for N_A in N_A_values:
            for _ in range(n_hamiltonians):
                ...
                A0 = quantum.haar_random_state(N_A, rng)
...
def main() -> int:
    ...
    max_diff = 0.0
    for r in results:
        diff = abs(r.p_mc - r.p_integral)
        max_diff = max(max_diff, diff)
        status = "PASS" if r.passes else "FAIL"
        print(
            f"  g_tau={r.g_tau:.1f}: eps={r.epsilon:.4f} "
            f"P_int={r.p_integral:.4f} P_MC={r.p_mc:.4f} "
            f"|diff|={diff:.4f} [{status}]"
        )

    n_pass = sum(1 for r in results if r.passes)
    print(f"\n{n_pass}/{len(results)} passed, max |diff|={max_diff:.4f}")
