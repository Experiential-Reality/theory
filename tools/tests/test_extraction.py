...
class TestExtractHeaders:
    def test_single_header(self):
        assert "hello-world" in list(extract_headers("# Hello World"))

    def test_multiple_levels(self):
        anchors = set(extract_headers("# Level 1\n## Level 2\n### Level 3\n"))
        assert {"level-1", "level-2", "level-3"} <= anchors

    def test_header_with_content(self):
        anchors = set(extract_headers("# Introduction\n\nText.\n\n## Methods\n"))
        assert anchors == {"introduction", "methods"}


class TestExtractLinks:
    def test_inline_link(self):
        links = list(extract_links("[link text](./file.md)"))
        assert len(links) == 1
    ...
    def test_link_with_anchor(self):
        links = list(extract_links("[section](./file.md#section-one)"))
        assert links[0].url == "./file.md#section-one"

    def test_internal_anchor_only(self):
        links = list(extract_links("[jump](#section)"))
        assert links[0].url == "#section"

    def test_skips_external_links(self):
        assert list(extract_links("[Google](https://google.com)")) == []

    def test_skips_image_suffixes(self):
        assert list(extract_links("[logo](./image.png)")) == []
    ...
    def test_multiple_links(self):
        assert len(list(extract_links("[one](./a.md) and [two](./b.md)"))) == 2

    def test_line_numbers(self):
        links = list(extract_links("Line 1\n[link](./file.md)\nLine 3\n"))
        assert links[0].line == 2


class TestExtractReferenceLinks:
    def test_reference_link_resolved(self):
        links = list(extract_links("[link text][ref1]\n\n[ref1]: ./file.md\n"))
        ref_links = [link for link in links if link.kind == LinkKind.REFERENCE]
    ...
    def test_undefined_ref_detected(self):
        links = list(extract_links("[text][undefined]"))
        undefined = [link for link in links if link.kind == LinkKind.UNDEFINED_REF]
        assert len(undefined) == 1
        assert undefined[0].ref == "undefined"

    def test_reference_in_code_block_ignored(self):
        assert list(extract_links("```\n[ignored][ref]\n```\n[ref]: ./file.md\n")) == []

    def test_case_insensitive_ref_lookup(self):
        links = list(extract_links("[text][REF]\n\n[ref]: ./file.md\n"))
        ref_links = [link for link in links if link.kind == LinkKind.REFERENCE]
        assert ref_links[0].url == "./file.md"

    def test_reference_link_format_for_error(self):
        links = list(extract_links("[text][ref]\n\n[ref]: ./file.md\n"))
        ref_link = next(link for link in links if link.kind == LinkKind.REFERENCE)
    ...
    def test_undefined_ref_format_for_error(self):
        links = list(extract_links("[text][undefined]"))
        undefined = next(link for link in links if link.kind == LinkKind.UNDEFINED_REF)
