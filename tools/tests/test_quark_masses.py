...
def test_strange_mass() -> None:
    """Strange quark mass from electron mass * (n^2S - L - L/n)."""
    ratio = tools.bld.ms_over_me(tools.bld.n, tools.bld.S, tools.bld.L)
    ms = tools.bld.M_ELECTRON * ratio
    obs = tools.bld.M_STRANGE
    assert_all_pass([tools.bld.Prediction("m_s(MeV)", ms, obs.value, obs.uncertainty)])
...
def test_down_mass() -> None:
    ...
    assert_all_pass([tools.bld.Prediction("m_d(MeV)", md, obs.value, obs.uncertainty)])
...
def test_up_mass() -> None:
    ...
    mu = md / ratio
    obs = tools.bld.M_UP
    assert_all_pass([tools.bld.Prediction("m_u(MeV)", mu, obs.value, obs.uncertainty)])
...
def test_charm_mass() -> None:
    ...
    assert_all_pass([tools.bld.Prediction("m_c(MeV)", mc, obs.value, obs.uncertainty)])
...
def test_bottom_mass() -> None:
    ...
    assert_all_pass([tools.bld.Prediction("m_b(MeV)", mb, obs.value, obs.uncertainty)])


def test_top_mass() -> None:
    """Top quark mass from v/sqrt(K) * (1 - K/n^2S)."""
    mt = tools.bld.top_mass(tools.bld.V_EW, tools.bld.K, tools.bld.n, tools.bld.S)
    obs = tools.bld.M_TOP
    assert_all_pass([tools.bld.Prediction("m_t(GeV)", mt, obs.value, obs.uncertainty)])
...
def test_quark_chain() -> None:
    ...
    assert_all_pass([tools.bld.TestResult("chain_all_6_pass", n_pass == 6, float(n_pass))])
...
def test_confinement_cost() -> None:
    ...
    In ms/me = n^2S - L - L/n, the negative terms (-L - L/n) represent
    the confinement cost that suppresses quark masses relative to the
    generational base n^2S = 208.
    """
...
def test_kx_hierarchy() -> None:
    ...
    K/L (link) > K/3 (color) > K/7 (n+3) > K/208 (n^2S).
