...
class TestValidateAnchor:
    ...
    def test_valid_anchor(self):
        ...
        link = Link(kind=LinkKind.INLINE, text="test", url="#section-one", line=1, pos=0)
        error = validate_anchor("section-one", anchors, pathlib.Path("test.md"), link)
        assert error is None

    def test_missing_anchor(self):
        anchors = {"section-one"}
        link = Link(kind=LinkKind.INLINE, text="test", url="#missing", line=1, pos=0)
    ...
    def test_similar_anchor_hint(self):
        anchors = {"section-one", "section-two"}
        link = Link(kind=LinkKind.INLINE, text="test", url="#sectionone", line=1, pos=0)
...
class TestFindSimilar:
    """Tests for fuzzy anchor matching."""
...
class TestResolveTarget:
    ...
    def test_relative_path(self):
        link = Link(kind=LinkKind.INLINE, text="test", url="./other.md", line=1, pos=0)
    ...
    def test_relative_path_with_anchor(self):
        link = Link(kind=LinkKind.INLINE, text="test", url="./other.md#section", line=1, pos=0)
        source = pathlib.Path("/docs/folder/file.md")
        target, anchor = resolve_target(link, source, pathlib.Path("/docs"))
        assert target == pathlib.Path("/docs/folder/other.md")
    ...
    def test_absolute_path(self):
        link = Link(kind=LinkKind.INLINE, text="test", url="/root/file.md", line=1, pos=0)
        source = pathlib.Path("/docs/folder/file.md")
        target, anchor = resolve_target(link, source, pathlib.Path("/docs"))
    ...
    def test_reference_link_resolution(self):
        ...
        link = Link(kind=LinkKind.REFERENCE, text="test", url="./other.md", line=1, pos=0, ref="ref1")
        source = pathlib.Path("/docs/folder/file.md")
        target, anchor = resolve_target(link, source, pathlib.Path("/docs"))
        assert target == pathlib.Path("/docs/folder/other.md")
