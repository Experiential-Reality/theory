...
| Born rule deviation from pointer non-orthogonality | **PREDICTED** | For M = 2 with pointer overlap ε = \|⟨O₀\|O₁⟩\|² and α² = (0.7, 0.3): Δ(ε) = P(f=0) − 0.7 ≈ 0.093ε² + 0.078ε (quadratic fit, RMS = 0.0017). Deviation always biases toward dominant outcome. For M = 3, Born rule fails χ² at ε ≥ 0.10. Testable in controlled quantum systems with weak decoherence — falsifiable prediction unique to BLD. See [Born Rule, Testability](../mathematics/quantum/born-rule.md#testability). |
