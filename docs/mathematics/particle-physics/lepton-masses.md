...
Euler's number e emerges from discrete traversal as e = lim(1+1/m)^m — the boundary where discrete becomes continuous. See [e from BLD](../../examples/e-from-bld.md) for the full derivation. The key BLD decomposition: **π** = rotational closure (D-type), **e** = discrete accumulation (L-type), and e is the boundary (B) where L becomes continuous.
