...
2. Zorn's classification limits options to ℝ, ℂ, ℍ, 𝕆 — [Classification: Zorn and Hurwitz](#3-classification-zorn-and-hurwitz)
...
We derive the octonion algebra, four-dimensional spacetime (n = 4), and SU(3) color symmetry from BLD first principles. The derivation proceeds as follows: (1) bidirectional observation requires invertibility and pairwise consistency (alternativity); (2) by Zorn's theorem [Zorn, 1930], only four alternative division algebras exist (ℝ, ℂ, ℍ, 𝕆); (3) genesis closure requires B = 56 modes, which only the octonions can support; (4) BLD observation requires fixing a reference point in the octonions, which simultaneously breaks G₂ → SU(3), reduces so(9,1) → so(3,1), and activates Spin(8) triality. This single symmetry breaking mechanism produces n = 4 spacetime dimensions, SU(3) color symmetry, and three particle generations.
...
3. **Pairwise consistency requires alternativity.** For any two observers a, b, observation must be self-consistent regardless of grouping: (aa)b = a(ab) and (ab)b = a(bb). If pairwise observation were not associative, the same pair of elements would yield different results depending on parenthesization — violating the determinacy required by A5 (traversal must have finite, well-defined cost). This property — alternativity — is equivalent to: every subalgebra generated by two elements is associative (Artin's theorem; see [Zorn, 1930]).

4. **Invertible + Alternative = Alternative Division Algebra.** A binary operation that is invertible and alternative is exactly an alternative division algebra. ∎

### 2.2 Formal Statement

**Theorem 2.2** (Division Requirement). BLD self-observation requires an alternative division algebra.
...
3. Pairwise observation consistency requires alternativity [from A5 determinacy]
4. By definition, this is an alternative division algebra ∎

**Corollary 2.3.** Without alternativity and invertibility, BLD observation would be either irreversible (no inverse) or ambiguous (pairwise inconsistency). Both violate the axioms.

## 3. Classification: Zorn and Hurwitz

**Theorem 3.1** (Zorn, 1930). The only finite-dimensional alternative division algebras over ℝ are:
...
| 𝕆 (octonions) | 8 | alternative, division (non-associative) |

*This is Zorn's classification [Zorn, 1930], which requires only alternativity — exactly what BLD forces (Theorem 2.2). There are no others.*

**Remark 3.1.1** (Relationship to Hurwitz). Hurwitz's theorem [Hurwitz, 1898] proves the same classification under the stronger assumption of a multiplicative norm. For finite-dimensional real division algebras, the multiplicative norm is a *consequence* of alternativity, not an independent axiom. Zorn's theorem is therefore the more natural result for BLD, since it derives from the weaker assumption that BLD actually forces.
...
**Corollary 3.3.** At sedenions (16D), zero divisors exist (ab = 0 with a, b ≠ 0) and alternativity fails. Some links would have no reverse and pairwise observation becomes ambiguous, violating BLD's alternativity and invertibility requirements.

**Conclusion.** Octonions are the *last* algebra where BLD observation works.

## 4. Why Octonions Specifically

**Theorem 4.1** (Octonion Necessity). Among the four alternative division algebras (ℝ, ℂ, ℍ, 𝕆), only octonions satisfy BLD genesis closure requirements.
...
- Zorn classification [Zorn, 1930] / Hurwitz theorem [Hurwitz, 1898]
...
The connection between division algebras and physics has been explored by many authors. The role of octonions in particle physics was pioneered by [Günaydin & Gürsey, 1973] and developed extensively by [Dixon, 1994]. The classification of alternative division algebras is due to [Zorn, 1930]; the equivalent classification of normed division algebras to [Hurwitz, 1898]. Modern treatments appear in [Baez, 2002] and [Schafer, 1995].
