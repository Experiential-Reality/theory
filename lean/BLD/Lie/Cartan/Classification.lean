      ...
      · -- Weight 1
        ...
        · -- 1 ≤ p ≤ k-2 (interior) → contradiction via subgraph null vector
          ...
          have hBpath_sum : ∀ i_off : Fin rr,
            ...
            intro i_off
            -- Split fB into three parts: q < p-1 → 0, q = p-1 → 1, q ≥ p → B_marks
            -- For q < p-1: fB(q) = 0, so the term vanishes
            -- For q = p-1: fB(q) = 1, B(i_off+p, p-1) = -1 if i_off=0, 0 else
            -- For q ≥ p: fB(q) = B_marks(rr, q-p), B(i_off+p, q) = B_rr(i_off, q-p) via B_shift
            -- So sum = B_left_contrib + ∑_k B_rr(i_off,k)*B_marks(rr,k)
            --        = (if i_off=0 then -1 else 0) + B_mulVec_marks(rr, i_off)
            --        = (if i_off=0 then -1 else 0) + (if i_off=0 then 2 else 0)
            sorry
