...
abbrev SkewIdx := {p : Fin 8 × Fin 8 // p.1 < p.2}

theorem card_skewIdx : Fintype.card SkewIdx = 28 := by native_decide

def skewMat (p : SkewIdx) : Matrix (Fin 8) (Fin 8) ℚ :=
  Matrix.single p.val.1 p.val.2 1 - Matrix.single p.val.2 p.val.1 1

theorem skewMat_mem (p : SkewIdx) : skewMat p ∈ so8 ℚ := by
  rw [mem_so8, skewMat, Matrix.transpose_sub, Matrix.transpose_single,
      Matrix.transpose_single, neg_sub]

def bv (p : SkewIdx) : ↥(so8 ℚ) := ⟨skewMat p, skewMat_mem p⟩

theorem skewMat_self (p : SkewIdx) :
    skewMat p p.val.1 p.val.2 = 1 := by
  simp [skewMat, Matrix.sub_apply,
        show p.val.1 ≠ p.val.2 from Fin.ne_of_lt p.property]

theorem skewMat_other (p q : SkewIdx) (hpq : p ≠ q) :
...
theorem sum_skewMat_entry (c : SkewIdx → ℚ) (a b : Fin 8) :
...
theorem skewMat_li : LinearIndependent ℚ skewMat := by
...
theorem sum_skewMat_entry_eq (c : SkewIdx → ℚ) (p : SkewIdx) :
...
theorem skew_reconstruct {A : Matrix (Fin 8) (Fin 8) ℚ}
...
-- Spanning
theorem bv_span : ⊤ ≤ Submodule.span ℚ (Set.range bv) := by
  intro ⟨A, hA⟩ _
