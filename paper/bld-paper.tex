...
seven further predictions---all with zero free parameters, most as
exact rational fractions and several involving rational corrections to
transcendental functions.  The same five constants predict the genetic code (4
...
\tfrac{27}{100}$, $\Omega_b = \tfrac{1}{20}$) matching Planck~2018 within $0.5\sigma$.  The mathematics is verified in Lean~4 with Mathlib: 26
...
\item \textbf{Predictions} (\S\ref{sec:predictions}): Compute 12 predictions and compare to experiment.
...
$-e^2 \cdot (2B+n+K+2)/((2B+n+K+1)(nL)^2 B^2) \approx -0.000001$,
...
$^\dagger$$v = 246.22$\,GeV (Higgs VEV) is taken from experiment; BLD
predicts the dimensionless correction factor, not the absolute energy
scale.  See Appendix~\ref{app:alpha} for the $\alpha\inv$ correction
details.}
\end{table}
...
(taken from experiment; BLD predicts the dimensionless correction
factor, not the absolute energy scale).
...
these five integers, with zero free parameters for dimensionless
quantities (the absolute energy scale $v$ remains an experimental
input), we derive:
