...
space dimensions).  The derivation chain is machine-verified in Lean~4
with Mathlib (26 files, 4360 lines, zero \texttt{sorry}, zero axioms).
...
26 files totaling 4360 lines of proof, with zero uses of \texttt{sorry}
(unproved assertion), zero uses of \texttt{admit} (axiom introduction),
and zero custom axioms.  Every theorem is proved from definitions.
...
No step involves empirical input.  The chain is logically forced.
...
BLD arithmetic, via the Freudenthal magic square and Tits construction:
...
a strict hierarchy ($B < n\!+\!L < B\!+\!L < nL$) with numerator $K =
2$ universal.  A skeptic seeking to dismiss the $K/X$ framework as
fitting must explain why the same five integers, combined through a
single deterministic algorithm, reproduce exact rational fractions
across electroweak, Higgs, and cosmological domains.
...
For black holes: the Bekenstein--Hawking entropy $S = A/(4\ell_P^2)$
...
\subsection{General Relativity from K/X}
\label{sec:general-relativity}

The $K/X$ observer correction framework directly produces the
structure of general relativity:

\begin{itemize}
\item The Schwarzschild radius $r_s = 2GM/c^2 = K \cdot GM/c^2$:
  the factor 2 is the Killing form.
\item Gravitational time dilation $\sqrt{1 - r_s/r}$ is a $K/X$
  correction with infinite stack depth at $r = r_s$.
\item The factor $8\pi$ in Einstein's field equations:
  $8\pi = K \times n \times \pi = 2 \times 4 \times \pi$, where
  $K = 2$ is the Killing form and $n = 4$ is spacetime dimension.
\item Geodesics are minimum traversal cost paths through BLD structure.
...
Table~\ref{tab:predictions} collects the 14 principal predictions.
...
$m_H$ & $\frac{v}{2}(1\!+\!\frac{1}{B})(1\!-\!\frac{1}{BL})$$^\dagger$ & --- & 125.20\,GeV & 125.20\,GeV & 0.0$\sigma$ \\
$\alpha_s\inv$ & $\frac{137}{n^2} - \frac{K}{n+L}$ & $\frac{407}{48}$ & 8.479 & 8.479 & 0.0$\sigma$ \\
$\text{Re}_c(\text{pipe})$$^\ddagger$ & $\frac{nLB}{K} \cdot \frac{X\!+\!1}{X}$ & $\frac{85120}{37}$ & 2300.5 & 2300(1) & 0.0$\sigma$ \\
$\delta$ (Feigenbaum)$^\ddagger$ & $\sqrt{L\!+\!K\!-\!K^2\!/L\!+\!e^{-X}}$ & --- & 4.66920 & 4.66920 & 0.0003\% \\
$\mu/e$ (mass ratio) & $n^2 S$ (corrected)$^*$ & --- & 206.768 & 206.768 & $<0.01\sigma$ \\
\midrule
\multicolumn{6}{l}{\textit{Additional predictions (see text):}} \\
$\alpha_F$ (Feigenbaum)$^\ddagger$ & $K + 1/K + \text{corr.}$ & --- & 2.50291 & 2.50291 & 0.0\% \\
$H_0(\text{local})$ & $H_0^{\text{CMB}} \!\times\! 13/12$ & --- & 73.0 & 73.0(10) & 0.0$\sigma$ \\
...
$^\dagger$$v = 246.22$\,GeV (Higgs VEV) is itself derived from BLD
...
details.\\
$^\ddagger$Cross-domain predictions; derivations in Appendix~\ref{app:turbulence}.}
\end{table}
...
\end{equation}
The tau--muon ratio has integer primordial $S + n = 17$, refined by
the continuous primordial $2\pi e \approx 17.079$ (the same structural
constant in exponential form):
\begin{equation}
...
= 67.4 \times \frac{13}{12} = 73.0\;\text{km/s/Mpc}
\label{eq:hubble}
\end{equation}
...
The predicted local value $73.0\;\text{km/s/Mpc}$ matches the SH0ES
measurement $73.0 \pm 1.0\;\text{km/s/Mpc}$~\cite{SH0ES2022}
exactly.  Both measurements are correct---they measure different
things.
...
\item $S/(S-1) = 13/12$: generation structure correction (the same
      factor as the Hubble tension, Eq.~\ref{eq:hubble}).
\end{itemize}
...
$H_0(\text{local})$ & 73.0\,km/s/Mpc & SH0ES/TDCOSMO & Ongoing & Confirmed \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Incompleteness}: Some areas remain undeveloped ($L$ cosmological scaling assumption, Born rule deviation experimental test).  The baryon asymmetry is now derived (\S\ref{sec:tensions}, Eq.~\ref{eq:baryon}), the $H_0$ absolute value is derived (\S\ref{sec:tensions}, Eq.~\ref{eq:h0-abs}), and the quantum gravity relationship is addressed structurally (BLD = Lie theory = QM language, \S\ref{sec:quantum}), but future work may reveal disagreements in unexplored domains.
\end{enumerate}
...
\item \textbf{Born rule deviation}: The predicted deviation from the Born rule at pointer non-orthogonality $\varepsilon \geq 0.10$ (Eq.~\ref{eq:born-deviation}) has not yet been tested experimentally.  This is a falsifiable prediction unique to BLD.
\end{enumerate}
...
\item The Hubble tension resolved: $H_0(\text{local}) = 73.0\;\text{km/s/Mpc}$.
...
not have navigated alone.  The author thanks his wife for tolerating
the ranting.
...
which the Riemann component count equals the amino acid count.  This is
not numerology---it is a structural constraint that selects $n = 4$
independently of the Killing form argument.
