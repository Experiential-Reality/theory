...
$\alpha\inv = 137.035\,999\,177$ (matching CODATA~2022 to all reported digits), the weak
mixing angle $\sin^2\!\theta_W = \tfrac{6733}{29120}$, the
proton--electron mass ratio $m_p/m_e = \tfrac{23870}{13}$, three PMNS
neutrino mixing angles (combined $\chi^2 = 0.008$, $p = 0.9998$),
all six quark masses ($<0.5\%$), and further predictions---all with
zero free parameters.  The absolute energy scale
$v = 246.22\;\text{GeV}$ is derived as the fixed point of
self-observation ($0.00014\%$ accuracy).  The same framework derives the
Born rule ($P = |\psi|^2$ from $K = 2$ bidirectional alignment),
resolves the measurement problem (collapse as $L \to B$ compensation
with an explicit selection rule exact for all Hilbert space dimensions),
resolves the Hubble tension ($H_0^{\text{local}} = 67.4 \times
13/12 = 73.0\;\text{km/s/Mpc}$, $0\sigma$), derives the baryon-to-photon
ratio $\eta \approx 6.05 \times 10^{-10}$ ($1.0\sigma$), and derives the
absolute Hubble constant $H_0 = v \cdot \lambda^{68} =
67.2\;\text{km/s/Mpc}$ ($0.4\sigma$) from BLD alone.  Cross-domain predictions
include the genetic code (4~bases, 20~amino acids, 61~coding codons),
three cosmological density fractions matching Planck~2018 within
$0.5\sigma$, and the cosmological constant problem dissolved via finite
structure.  The mathematics is verified in Lean~4 with Mathlib: 26
...
neutrino normal mass ordering (JUNO), neutron beam lifetime 888.2\,s
(BL3/J-PARC), and Born rule deviation at pointer non-orthogonality
$\varepsilon \geq 0.10$.
\end{abstract}
...
\item \textbf{Constants} (\S\ref{sec:constants}): Derive $K = 2$ from the Killing form; obtain $n$, $L$, $S$, $B$ via identities. Prove $K = 2$ is unique. Derive the reference scale $v = 246.22\;\text{GeV}$.
...
\item \textbf{Cross-domain} (\S\ref{sec:genetic}--\ref{sec:cosmology}): Apply the same five constants to the genetic code, cosmological fractions, and cosmological tensions.
...
knowledge update.  It is the compensation principle operating on
quantum states.
...
$|\alpha_k|^{2/\tau}/Z$ that disagree with experiment.
...
The formula $S = K \times L = 2L$ unifies entropy across domains:
...
$CPT$ conservation follows from $K = 2$ constancy: a bidirectional
observation must be unchanged under complete reversal of all three
primitives.  Individual $C$ and $P$ violations arise because $+B \neq -B$
...
$\alpha_F$ (Feigenbaum) & $K + 1/K + \text{corr.}$ & --- & 2.50291 & 2.50291 & 0.0\% \\
...
Derives $\alpha\inv$? & No (input from experiment) & Yes (137.036) \\
Derives mixing angles? & No (input from experiment) & Yes ($\chi^2 = 0.008$) \\
Derives mass ratios? & No (input from experiment) & Yes ($m_p/m_e$, $\mu/e$, $\tau/\mu$) \\
...
Measurement problem & Unresolved & Derived ($L \to B$ compensation) \\
...
\begin{enumerate}
\item \textbf{Baryon asymmetry} \emph{(resolved)}: Now derived as $\eta = (K/B)(1/L)^6 \cdot S/(S-1) \approx 6.05 \times 10^{-10}$ ($1.0\sigma$ from Planck~2018); see Eq.~\eqref{eq:baryon}.

\item \textbf{$H_0$ absolute value} \emph{(resolved)}: Now derived as $H_0 = v \cdot \lambda^{68}$ giving $67.2\;\text{km/s/Mpc}$ ($0.4\sigma$ from Planck~2018); see Eq.~\eqref{eq:h0-abs}.
