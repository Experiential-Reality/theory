...
with Mathlib (24 BLD files, 3522 lines, plus 12 Cartan classification
files, 7439 lines; zero \texttt{sorry}, zero axioms).
...
type-theoretic constructors---the same \emph{sum}, \emph{function},
and \emph{product} that underlie every typed programming language and
every categorical topos---and ends with exact rational predictions
for physical constants.  The three constructors correspond to three
...
and these five integers suffice to compute every physical constant we
examine.
...
comprises 24 files totaling 3522 lines, supplemented by a complete
Lean formalization of the Cartan classification (12 files, 7439 lines)
proving every indecomposable positive-definite generalized Cartan
matrix is one of 9 Dynkin types.  The combined 36 files (10961 lines)
...
\item \textbf{Cartan classification}: Every indecomposable positive-definite generalized Cartan matrix is one of 9 Dynkin types ($A_n$, $B_n$, $C_n$, $D_n$, $E_6$, $E_7$, $E_8$, $F_4$, $G_2$), proved by forbidden subgraph analysis, Coxeter weight bounds, and rank induction (12 files, 7439 lines, 0~\texttt{sorry}).
\end{enumerate}
...
\item \textbf{Gauge and generation structure}: The $\uu(4)$ closure
  (Theorem~\ref{thm:gauge}), hypercharge computation,
  $\Der(\mathbb{H})$ derivation (Theorem~\ref{thm:weak}), and
  Casimir--curvature bridge (Theorem~\ref{thm:casimir}) are verified
  numerically but not yet formalized in Lean.
\end{enumerate}
...
The mathematics is machine-verified: 24 BLD files (3522 lines) plus a
complete Cartan classification (12 files, 7439 lines)---36 Lean~4 files,
10961 lines, zero \texttt{sorry}, zero axioms.
