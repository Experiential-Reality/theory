...
\item \textbf{Incompleteness}: Some areas remain undeveloped ($L$ cosmological scaling assumption, Born rule deviation experimental test).  The baryon asymmetry is now derived (\S\ref{sec:tensions}, Eq.~\eqref{eq:baryon}), the $H_0$ absolute value is derived (\S\ref{sec:tensions}, Eq.~\eqref{eq:h0-abs}), and the quantum gravity relationship is addressed structurally (BLD = Lie theory = QM language, \S\ref{sec:quantum}), but future work may reveal disagreements in unexplored domains.
\end{enumerate}
